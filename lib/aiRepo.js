...
import { getRequests, getRequest } from "./requestStore";
...
export function summarizeRecord(record) {
  ...
  // Compute expiry from effectiveDate + term (rough — for year/month suffix).
  let expiry = null;
  if (f.effectiveDate && f.term) {
    const m = String(f.term).match(/(\d+)\s*(year|month)/i);
    if (m) {
      const n = parseInt(m[1], 10);
      const unit = m[2].toLowerCase();
      const d = new Date(f.effectiveDate);
      if (!isNaN(d)) {
