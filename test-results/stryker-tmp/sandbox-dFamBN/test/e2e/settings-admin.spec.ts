...
test("Admin can access settings, add users, and switch themes", async ({ page }) => {
  ...
  await expect(page.getByText("Carter")).toBeVisible();
});
...
test("Admin can manage Parties and Rules and dropdowns are readable", async ({ page }) => {
  ...
  await expect(page.getByText("QA Partner LLC")).toBeVisible();
  ...
  await expect(page.getByText("QA High Risk Escalation")).toBeVisible();
});
