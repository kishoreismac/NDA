...
test("newly created Admin templates are available during NDA creation", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /new template|add template|create template/i }).first().click();
  await page.getByPlaceholder(/template name|name/i).first().fill("QA Admin NDA Template");
