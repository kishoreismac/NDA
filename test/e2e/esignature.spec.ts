...
async function createSigningLink(page) {
  ...
  await page.getByTestId("repository-view").first().click();
  await expect(page.getByTestId("repository-detail-drawer")).toBeVisible();
  await expect(page.getByTestId("repository-detail-send-for-sign")).toBeEnabled();
...
test("counterparty can open signing page, preview/download, sign, and see confirmation", async ({ page, context }) => {
  ...
  await signer.getByTestId("signature-mode-type").click();
  await signer.getByTestId("signature-typed-name").fill("Morgan QA Ellis");
