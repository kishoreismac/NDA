...
describe("e-signature status transitions", () => {
  ...
  it("blocks send for signature until the NDA is approved", async () => {
    upsertRequest({
      id: "NDA-9103",
      title: "Unapproved Signature NDA",
      status: "In Review",
      form: { counterpartyName: "Acme", counterpartyEmail: "signer@acme.com" },
    });

    const result = await sendForSignature("NDA-9103");
    expect(result.ok).toBe(false);
    expect(result.error).toMatch(/approved/i);
    expect(getRequest("NDA-9103").status).toBe("In Review");
  });
